...
null_model <- lm(price_per_m2 ~ 1, data = property_data_cleaned2_subset)  # Null model with only intercept
full_model <- lm(price_per_m2 ~ ., data = property_data_cleaned2_subset)  # Full model with all features
...
property_data_cleaned3 <- property_data_cleaned2_subset[-outlier_indices, ]
